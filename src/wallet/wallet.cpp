    ...
    int height) const
{
    ...
    for (const auto& output : tx.GetSaplingOutputs()) {
        bool found = false;
        // First pass: try every full viewing key. FVKs subsume the ivk capability
        // and are the common case (any address generated by this wallet has an FVK).
        ...
        // Second pass: try ivk-only entries (imported via z_importivk) that
        // are NOT also covered by an FVK from the first pass. Without this,
        // imported viewing keys would be inert -- their addresses would never
        // see incoming notes during scanning. Mirrors ycash-official.
        if (!found) {
            for (const auto& [addr, ivk] : mapSaplingIncomingViewingKeys) {
                // Skip ivks that the FVK pass already tried.
                if (mapSaplingFullViewingKeys.count(ivk) > 0) {
                    continue;
                }

...
unsigned int nDbWriteInterval = DATABASE_WRITE_INTERVAL;
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (fAddressIndex && updateIndices) {
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                auto [scriptType, addrHash] = out.scriptPubKey.AddressIndexKey();
...
bool static FlushStateToDisk(
    ...
    bool fPeriodicWrite = mode == FLUSH_STATE_PERIODIC && nNow > nLastWrite + (int64_t)nDbWriteInterval * 1000000;
    ...
    const CChainParams& chainParams, CBlockIndex * const pindexPrev)
{
    ...
    // The remaining checks here are header-only (difficulty, timestamp,
    // checkpoint position, version): they depend on the header and chain
    // context, never on the block body, so failures are not body-replaceable
    // and pass `BodyCorruption::HeaderOnly`.

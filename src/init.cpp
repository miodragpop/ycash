...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindex-one-phase", strprintf(_("Connect each block to the active chain immediately after accepting it during reindex, instead of accepting all blocks first and then activating the best chain at the end. This avoids reading every block file twice and is faster on RAM-constrained or slow-disk nodes; it is functionally equivalent. Set to 0 for the older two-phase reindex (accept all, then connect). (default = %u)"), DEFAULT_REINDEX_ONE_PHASE));
    ...
    strUsage += HelpMessageOpt("-listenonion", strprintf(_("Automatically create a legacy Tor v2 hidden service (default: %d). Note: Tor v2 onion services were disabled by the Tor network in 2021, so this is non-functional; kept only for legacy compatibility."), DEFAULT_LISTEN_ONION));
